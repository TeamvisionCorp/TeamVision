...
from doraemon.project.models import IssueActivity, Project
from gatesidelib.common.simplelogger import SimpleLogger
from gatesidelib.datetimehelper import DateTimeHelper
from doraemon.project.models import ProjectIssue, IssueFilter
from doraemon.project.mongo_models import IssueMongoFile
...
from doraemon.project.viewmodels.vm_project_issue import VM_ProjectIssue
...
class IssueService(ExcelFileService,BusinessService):
    ...
    def create_issue(request):
        try:
            issue = ProjectIssue()
            issue = IssueService.init_issue(request, issue)
            issue.Creator = request.user.id
            issue.Status = 2
            issue.Solution = 1
            issue.save()
            IssueService.send_notification_email(issue,"创建了新问题",EMAIL_TEMPLATES['Issue'])
            IssueService.create_issue_activity(issue, None, None, None, "新问题", request.user.id, 1, 1)
            IssueService.log_create_activity(request.user, issue)
    ...
    def update_issue_operation_result(issue_id, operation_type, solution, comments, user_id):
        ...
        try:
            ...
            if str(operation_type) == "1":  # 解决问题
                ...
                IssueService.create_issue_activity(issue, field, solution, old_value, "", user_id, 2, 1)
            ...
            if str(operation_type) == "2":  # 关闭问题
                ...
                issue.UpdateTime = DateTimeHelper.getcnow()
                issue.save()
                IssueService.create_issue_activity(issue, field, value, old_value, "", user_id, 2, 1)
            ...
            if comments != "":
                IssueService.create_issue_activity(issue, None, None, None, "备注" + comments, user_id, 1, 1)
    ...
    @staticmethod
    def init_issue(request, issue):
        tmp_issue = issue
        tmp_issue.Project = request.POST.get("ProjectID")
        tmp_issue.Version = request.POST.get("VersionID")
        tmp_issue.Title = request.POST.get("IssueTitle")
        tmp_issue.Desc = request.POST.get("IssueDescription")
        tmp_issue.Processor = request.POST.get("IssueProcessor", 0)
        tmp_issue.Team = request.POST.get("IssueTeam", 0)
        if str(tmp_issue.Processor) == '0':
            tmp_issue.Processor = request.user.id
        tmp_issue.Module = request.POST.get("ProjectModule")
        tmp_issue.IssueCategory = request.POST.get("IssueCategory")
        tmp_issue.Severity = request.POST.get("IssueSeverity")
        tmp_issue.ProjectPhase = request.POST.get("ProjectPhase")
        tmp_issue.DeviceOS = request.POST.get("OS")
        tmp_issue.OSVersion = request.POST.get("OSVersion")
        cached_file_keys = request.POST.get("attachments")
    ...
    @staticmethod
    def issue_excel_file(project_id, user_id):
        wb = xlwt.Workbook(encoding='utf-8')
        sheet_all = wb.add_sheet('ALL')
        sheet_all = IssueService.issue_all_sheet(sheet_all, project_id, user_id)
        return wb

    @staticmethod
    def issue_all_sheet(excel_sheet, project_id, user_id):
        all_issues = IssueService.all_issues(project_id, user_id)
