...
#         menuitemlist.append(item)
        autotask_active="leftmeunactive"
        return render_to_response('home/home_index_left_nav.html',{'meunitemlist':menuitemlist,'autotask_active':autotask_active},context_instance=RequestContext(request))
    except Exception as ex:
        print(ex)
        
def load_left_sub_navigater(request):
    try:
        return render_to_response('autotask/home_autotask_leftsub_nav.html',context_instance=RequestContext(request))
    except Exception as ex:
        print(ex)

def load_autotask_list(request):
    try:
        return render_to_response('autotask/home_autotask_listview.html',context_instance=RequestContext(request))
    except Exception as ex:
        print(ex)
